...
export default function EditAdsPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/ads`)
      .then((res) => res.json())
      .then((data) => {
        const ads = data.find((n: any) => n.id === id);
        if (ads) {
          setTitle(ads.title);
          setDescription(ads.description);
          setImage(ads.image || "");
          setStartDate(ads.startDate ? ads.startDate.slice(0, 10) : "");
          setEndDate(ads.endDate ? ads.endDate.slice(0, 10) : "");
          setIsPinned(ads.isPinned || false);
        } else {
